<?	require_once "inc/db_inc.php";
	...
	if ($download) {
		...
		if (count($search_results) > 0) {
			...
			PDF_set_info($mypdf, "Creator", "www.donate2educate.com");
			...
			while (list($projectid, $project) = each($search_results))
			{
				...
				PDF_continue_text($mypdf, "Request Cost: ".sprintf("%01.2f",$project->amount_needed));
				PDF_continue_text($mypdf, "Donations Received: ".sprintf("%01.2f",$project->amount_donated()));
				if ($project->amount_pledged())
				{
					PDF_continue_text($mypdf, "Donations Pledged: ".sprintf("%01.2f",$project->amount_pledged()));
				}
				PDF_end_page($mypdf);
			}
