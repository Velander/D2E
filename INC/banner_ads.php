...
<?
	...
	while (!$left_banner && list($bannerid, $lastdate) = each($banners)) {
		if ($banner->load_banner($bannerid)) {
			$banneruser = new user;
			$banneruser->load_user($banner->user_id);
			$banner->update_last_displayed($bannerid, $UserID, $pagename, $f_project_id)
?>
				<td align="center">
<?
					if ($banneruser->half_banner_link) {
						if ($banneruser->url) echo "<a href=\"banner_click.php?bannerid=$bannerid&pagename=$pagename&projectid=$projectid"."\" target=\"sponsor\">";
?>
					<img src="<?=(empty($HTTPS)?$http_location:$https_location).$banneruser->half_banner_link;?>" border=0 alt="<?=$banneruser->company;?>">
<?
						if ($schools->sponsor_url($f_school_id)) echo "</a>";
						$left_banner = 1;
					} elseif ($banneruser->banner_link) { ?>
					<img src="<?=(empty($HTTPS)?$http_location:$https_location).$banneruser->banner_link;?>" border=0 alt="<?=$banneruser->company;?>">
<?
						if ($schools->sponsor_url($f_school_id)) echo "</a>";
						$left_banner = 1;
					}
