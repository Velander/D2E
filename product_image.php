...
#
# $Id: product_image.php,v 1.13.2.4 2003/08/01 14:01:58 mclap Exp $
#
...
if ($tmp) {
    ...
    if (!empty($file_upload_data["file_path"]) && $file_upload_data["imtype"]=="D") {
...
if (empty($image_out)) {
	...
	$result=db_query("select image, image_path, image_type from $sql_tbl[images] where imageid='$imageid'");
	if (db_num_rows($result))
		list($image, $image_path, $image_type) = db_fetch_row($result);
	
	db_free_result($result);

	if ($config["Images"]["det_images_location"] == "DB") {
	    if (!empty($image))
	        $image_out = $image;
	    else
	        $no_image_db = true;
	...
	if ($config["Images"]["det_images_location"] == "FS" || $no_image_db) {
	    if (!empty($image_path)) { 
